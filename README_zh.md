...
### 翻译可靠性、审阅收口与编辑态一致性（v3.0.5）

- **翻译增量补译与缺口对账** -- 部分批次失败不再整任务报废：完成的批次落盘生效（1/N 降级落盘），失败段的缺口在面板明示（mm:ss 时间 + 20 字定位预览 + 一键补译，只重跑缺口差集，单 patch 合并 revision 只 +1）；重译同语言不再硬拒，自动路由为补译并 toast 明示补译 N 段。
- **质量模式（翻译定稿滑动窗）** -- 设置中 `llm_translation_quality_mode` 开关（默认关）：开启后翻译改串行派发，批 N 携带批 N-1 的定稿译文作上下文（窗口 1 批），跨批一致性更好；**时延约为并发模式的 5 倍**，按需取舍；补译同受该开关约束。
- **编辑模式冻结矩阵** -- 编辑模式 = 文本校对独占态：副轨块 trim 与副轨结构操作（建段/清空轨/删除轨）冻结并提示「请退出编辑模式后重试」；**主轨块 trim/菜单/手势维持 2.x 基线不受限——主/副轨不对称是刻意结果**（主轨为不可动基线，副轨为新增面按新规则）。
- **覆层三态语义** -- 手动范围覆层按 动作×状态 呈现：颜色 = 动作（红删除/蓝保留），透明度 = 状态（待确认半透明）；hover 提示中文语义化（如「保留范围 12.0s - 15.0s（待确认）」），keep 与删除区间重叠时附注「与重叠区间导出按删除处理」；**已拒绝（rejected）范围不再渲染**（退场，视觉噪音归零）。
- **取消中性提示与成本可见** -- 大工程翻译/纠错取消约 1s 返回（1s 轮询 + 非阻塞 shutdown）；翻译取消/失败 toast 中性化并附已消耗 token 量级（「翻译已取消，已消耗约 X tokens」），失败先上报成本再报错，不再制造"白跑一趟"的错觉。
