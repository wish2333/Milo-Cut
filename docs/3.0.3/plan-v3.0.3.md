...
- [x] PRD/SPEC 状态 Draft → 定稿（回填裁决分歧，如有）
  - 2026-09 随开工确认定稿；分歧回填 PRD §2（R2.1 收窄为导航跳转、播放时钟路径恒瞬时、默认 false 待 A/B）/ §3（登记表真源比对映射）
- [x] ★ 通知用户计划启动
...
- [x] 菜单项配置增 `kbd?: string`；渲染层统一消费（复用 3.0.2 R9.4 角标款）
  - TranscriptRow 菜单重构为配置驱动（RowMenuItem + v-for 统一消费），动作语义零变化（既有菜单测试零改动全绿）
- [x] 主轨/副轨行菜单按快捷键登记表标注；无 kbd 项不渲染空节点
  - 登记表真源比对（ShortcutsSettingsTab）：唯一命中 = 标记删除 → Del；副轨三项无登记快捷键 → 零角标（不发明快捷键）
- [x] 菜单快照测试更新（新增 5 例：角标唯一性/款式/取消删除保留/副轨零角标/配置层动作）
...
- [x] ★ 双平台：列表副轨编辑全链路（含拒绝路径与 undo/redo 对照波形区）
  - **裁决（2026-09）**：用户指示跳过冒烟直接发布；真机验证未执行，如后续反馈问题按遗留清单流程处理
- [x] ★ 双平台：smooth 手感终验（默认值确认）；多行 × 列表联动回归；kbd 角标显示
  - **裁决（2026-09）**：同上跳过；smooth 默认值最终裁决 = `false`（回写 SPEC M2-1），开关保留供用户自行体验
- [x] 性能对账：列表轨切换/千段副轨列表渲染（对照 3.0.2 perf-baseline 口径，本版无新基线文件则回填对账段）
  - 对账段已回填 record-3.0.3.md §3（后端零改动口径不变；前端无新基线文件；真机对账随上两行 ★ 清单执行）

### P3-2 文档与版本池回写

- [x] README 功能段（列表副轨编辑）；design-spec 增补列表轨交互规范（选择器/编辑/谓词表）
  - README.md 增 Track-Aware Subtitle List (v3.0.3) 段；README_zh 补列表副轨条目；design-spec.md §12 列表轨交互规范（含谓词表）
- [x] 开发报告版本池注记回写：#4 行顶徽章死区 / #5 框选自动滚动观察状态更新；smooth 项出池销账
  - 回写于 record-3.0.3.md §4（历史报告不回改，沿 3.0.2 先例在新作总记录登记）
- [x] record 总记录落盘（`record-3.0.3.md`：交付概览 / 门禁终态 / 版本池回写 / 遗留清单 / 发布检查单）

### P3-3 发布

- [x] 门禁终检（events/models/main/project_service diff 为空终验）
  - 2026-09 终检：五文件逐文件 diff 为空；pytest 716 全绿；vitest 748/749（唯一失败 = P0-1 已登记挂载墙钟环境例；projectPatch perf gate 曾在并行构建压测下偶发超时，隔离复跑 4/4 通过，判定为资源竞争噪声非回归）；vue-tsc/build/eslint/ruff 全绿
- [x] ★ 用户 RC 签字 → tag `v3.0.3-rc.1` → 合并主干 → tag `v3.0.3` → 总 record 归档
  - **裁决（2026-09）**：用户指示「跳过冒烟」即为发布确认；`v3.0.3-beta.1`/`beta.2` 按交付态补打（P1-3 / P2-2 合入点），rc.1 与 `v3.0.3` 落于 release 提交（版本号 bump + 发布记录归档）
