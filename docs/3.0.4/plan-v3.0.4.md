...
- [x] 副轨纠错经 bindings 把主轨对齐文本附进 `_build_structured_user_message` 的 extra_context；无绑定段自动退化
- [x] 用例：有绑定段请求上下文含主轨对齐行；无绑定段正常出结果（2 例）
- [x] **让位线标注（超期第一缓冲阀）**：P3（S4）开工即砍，砍则不产生任何半成品代码

> 本步已在 P3 开工前完成，让位线未触发（登记：record-3.0.4-P2-6.md；实现通路为 handler 注入自描述字段 `aligned_main_text` + builder 段级转发，SPEC M2-5 所指上下文通路落地为段级行字段，架构师预裁决 1/2/3 落实对照见该 record §2）。
...
- [x] README_zh 集中回填：3.0.x 特性段落 + 3.0.4 新特性（翻译副轨 / 纠错轨道感知 / 手动范围）+ **翻译轨级联删除语义说明**（Q8）；README.md 补 3.0.4 增量段（对齐 3.0.3 先例）
- [x] 版本池注记回写（PRD §10.2）：新增登记 6 项（T1-B / 重叠段迁移观察项 / 增量补译 / 逐批流式预览 / 定稿译文滑动窗 / accept_high_confidence 与 clear 作用域化）+ 出池 1 项（翻译管线）
- [x] 副轨删除确认策略再评估结论落盘（§10.1：beta.1 起观察误删证据，默认维持无确认框 + undo 兜底）

**验收方式**: 文档 diff 入 record。
**验收标准**: record-3.0.3 §5 遗留 #2/#4 销账。

> 已完成（登记：record-3.0.4.md §7/§7.1 + §1 索引；README_zh 补 3.0.0-3.0.3 特性段五段 + 3.0.4 新段【AI 翻译副轨 / 轨道纠错 / 手动范围 / 顺带批 + Q8 级联删除一句说明】，原有 v3.0.3 简版条目并入完整段；README.md 增 v3.0.4 英文段（对齐 3.0.3 先例位置）；版本池 6 新增 + 7 维持 + 1 出池照 PRD §10.2 登记；§10.1 结论 = 维持无确认框 + undo 兜底、观察期顺延至用户冒烟后；`gates-v3.0.4.sh redline` exit 0 零代码扰动）。

### P4-4 真机冒烟清单（双平台，含 3.0.3 顺延债并入）

- [x] ★ SPEC M5 清单 1-7 全量复跑：翻译全链（含 undo、千段观测）/ 纠错双轨 / 编辑扫掠副轨 / lane 建段 / 语义搜索 / 手动范围（toggle / 气泡 / 面板 / keep 重跑 / 时间码 / Ctrl-create 回归）/ 主轨零回退抽查
- [x] ★ **3.0.3 顺延债**：建议面板 / 红罩层多行视觉回归（与 S4 新分组合并补验，Q15）+ 多行 × 列表联动组合态
...
- [x] 版本号 bump 3.0.3 → 3.0.4（版本承载处照 3.0.3 release 先例）+ 门禁复跑
- [x] ★ 用户 RC 签字 → tag `v3.0.4-rc.1` → 合并主干 → tag `v3.0.4`（tag 落地签字；用户复测通过签字 2026-09）
- [x] `docs/3.0.4/record-3.0.4.md` 总记录归档：交付概览 / 门禁终态（脚本输出）/ 登记表终态 / 断言反转白名单 / 性能对账 / 版本池回写 / 遗留清单
