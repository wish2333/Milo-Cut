...
# Milo-Cut 1.0.0 打包修复记录 (2026-05-27)

## Windows subprocess 控制台弹窗抑制

**问题**: 所有 `subprocess.run()` 调用在 Windows 上会弹出系统终端窗口。拖入视频时 `nvidia-smi` (GPU检测) 和 `ffprobe` (媒体探测) 各弹一次窗口，且弹窗抢夺焦点导致第一次拖放失效。

**修复**: 所有 `subprocess.run()` 调用添加 `creationflags=subprocess.CREATE_NO_WINDOW` (仅 Windows 生效):

| 文件 | 调用点 |
|------|--------|
| `main.py` | `detect_gpu` -- nvidia-smi |
| `core/ffmpeg_service.py` | `probe_media` -- ffprobe; `detect_silence` -- ffmpeg; `generate_waveform` -- ffmpeg |
| `core/export_service.py` | `export_video`; `export_audio`; `export_srt`; `_extract_segment` |
| `core/ffmpeg_presets.py` | `check_encoder_availability` -- ffmpeg -encoders |

统一使用 `_SUBPROCESS_KWARGS` 常量，按平台条件注入。

## OTIO 插件清单打包修复

**问题**: PyInstaller 打包后 OTIO 导出失败 -- `[Errno 2] No such file or directory: '.../builtin_adapters.plugin_manifest.json'`。`opentimelineio` 依赖 JSON 插件清单注册其 OTIO adapter，但 PyInstaller 不自动收集 site-packages 中的非 Python 数据文件。

**修复**:

| 文件 | 变更 |
|------|------|
| `app.spec` | 在 `datas` 列表中动态探测并添加 `opentimelineio/adapters/builtin_adapters.plugin_manifest.json` |
| `build.py` | onefile spec 生成器同步添加相同数据文件 |

目标路径使用相对路径 `"opentimelineio/adapters"` (PyInstaller 要求 DEST_DIR 为相对路径)。

## Files Modified

| 文件 | 变更 |
|------|------|
| `main.py` | 新增 `_SUBPROCESS_KWARGS`; detect_gpu 添加 CREATE_NO_WINDOW |
| `core/ffmpeg_service.py` | 新增 `_SUBPROCESS_KWARGS`; 3 处 subprocess.run 添加 CREATE_NO_WINDOW |
| `core/export_service.py` | 新增 `_SUBPROCESS_KWARGS`; 4 处 subprocess.run 添加 CREATE_NO_WINDOW |
| `core/ffmpeg_presets.py` | 新增 `_SUBPROCESS_KWARGS`; check_encoder_availability 添加 CREATE_NO_WINDOW |
| `app.spec` | 添加 opentimelineio 插件清单到 datas |
| `build.py` | onefile spec 添加 opentimelineio 插件清单 |

## Commit Message

```
fix(build): 抑制 Windows subprocess 控制台弹窗 + 修复 OTIO 插件清单打包

- 所有 subprocess.run 添加 CREATE_NO_WINDOW，解决拖放视频时弹窗抢焦点问题
- app.spec/build.py 添加 opentimelineio plugin_manifest.json 到 PyInstaller datas
```

---
  Merge Message

  feat: Milo-Cut 1.0.0 -- 完整视频预处理工具首次发布

  合并 dev-0.2.0 / dev-0.2.1 / dev-0.2.2 分支，包含导出系统、波形编辑、
  静音检测、编码参数系统等全部功能开发和 Bug 修复。

  主要功能:
  - 视频/音频/SRT 导出，支持 H.264/H.265/VP9/NVENC 多编码器
  - OTIO/EDL/XML 时间线导出，兼容达芬奇和 Premiere Pro
  - FFmpeg 视频/音频过渡系统 (xfade + acrossfade)
  - 波形显示、持久化、按项目隔离、事件驱动加载
  - 静音检测增强: margin 缩边、字幕保护 padding、min duration 调整
  - 编码器注册表: 动态质量参数、像素格式探测、硬件编码器自动回退
  - 导出页面: 编码设置、实时预览、Toast 通知系统
...
---
  ...
  AI 驱动的桌面视频预处理工具首次正式发布。Milo-Cut 面向口播视频创作者，提供静音检测、智能剪辑、多格式导出一站式工作流。

  核心功能

  视频导出
  - 支持 H.264 (libx264)、H.265 (libx265)、AV1 (libsvtav1) 软件编码
  - 支持 NVIDIA NVENC 硬件加速 (h264_nvenc / hevc_nvenc / av1_nvenc)
  - 可配置质量 (CRF/CQ/QP)、分辨率、编码速度预设、音频编码器和码率
  - HDR/10-bit 输入自动保留原始像素格式
  - MP4/MOV 输出自动添加 -movflags +faststart 优化网络播放
  - 硬件编码器不可用时自动回退到 CPU 编码器

  时间线导出
  - OTIO 格式: OpenTimelineIO 标准，支持 PR 2025+ 和达芬奇 18+，带 SMPTE_Dissolve 过渡
  - XML 格式: FCP 7 XML (xmeml v5)，兼容 Premiere Pro 和达芬奇
  - EDL 格式: CMX3600，达芬奇专用
  - 支持 Clean Export 和 Full Timeline 两种模式
  - 视频/音频过渡时长独立控制

  音频导出
  - 独立音频文件导出 (AAC/Opus/MP3)
  ...
  FFmpeg 视频/音频过渡
  - 视频: xfade 链式交叉溶解
  - 音频: crossfade 交叉溶解 / separate 独立淡入淡出
  - 短片段自动 clamp 过渡时长，保证音视频同步
  ...
  - 波形持久化到磁盘，重启后自动加载
  - 事件驱动加载机制，适配长文件生成
  - 波形滑块拖动性能优化 (requestAnimationFrame 节流)
  - 波形重新生成按钮

  静音检测
  - FFmpeg silencedetect 驱动
  - Margin 缩边: 静音区间两侧收缩，避免误切
  - 字幕保护 Padding: 静音主动避让字幕区域
  - Min Duration 标尺: 0.05-2.0s 范围，短时静音性能警告
  - 流水线: FFmpeg 静音检测 -> margin 缩边 -> 字幕保护裁剪 -> 创建 Segment -> 去重 -> 保存

  导出页面 UI
  - 编码设置面板 (编码器/质量/分辨率/音频)
  - 实时预览播放器 (带波形、剪辑区域标记、音量控制)
  - 导出进度显示
  - Toast 通知系统 (成功/失败/编码器回退)

  已知限制

  - EDL 格式仅达芬奇可用，Premiere Pro 无法自动链接素材
  - XML 导入达芬奇需要手动确认音频链接
  - 硬件编码器检测仅支持 NVIDIA GPU
