...
macOS（WKWebView，2026-08-31 用户实测）已回填并通过；**Windows（WebView2）待后续补测**（用户裁决 2026-08-31：以 macOS 侧结果先行放行 beta.2，Windows 补测后回填本表）。

| 指标 | 目标 | macOS / WKWebView | Windows / WebView2 |
|---|---|---|---|
| 1167+ 段列表滚动帧率 | ≥ 55 fps | **60 fps ✅**（1200 段项目） | **待补测** |
| 波形生成任务期主线程长任务 | >50ms 长任务为 0 | **0 ✅**（PerformanceObserver） | **待补测** |
| 空闲 IPC 频率 | < 4 次/秒 | **≈4 次/秒 ✅**（累计读数每秒 +4 = 速率 4/s，250ms 降档生效） | **待补测** |
| 播放中 CPU 占用 | 对比 beta.1 下降 | 用户体感流畅 ✅（未量化） | **待补测** |
| 播放头无抖动 / hover 预览手感 | 对比 beta.1 截录屏 | 正常 ✅ | **待补测** |
| 波形缓存二次打开 < 200ms | M11-3 落地后测 | 未开始（Phase 4） | 未开始 |
| 1167 段单字编辑重渲染行数 ≤ 可视区 | Vue DevTools 高亮验证 | 待测 | 待测 |
| undo/redo ×5 手感 + macOS Cmd 链路 | 冒烟清单第 6 项 | 撤销重做正常 ✅（编辑/撤销即时无卡死） | **待补测** |
| 千段跳转定位（搜索/建议跳远端行） | 先瞬时定位无跳变 | 正常 ✅ | **待补测** |
| words 注入后吸附拆分（M1-4） | toast 吸附提示 + words 随拆分保留 | 夹具已注入（scripts/fabricate_words.py，1204 段/18162 词） | **待补测** |

## 结论

自动化可测的全部达标：undo 主线程 p50 1.3–2.9ms（目标 5ms）、千段 50/50 回放正确、打开/保存毫秒级无回归。真机冒烟：macOS（WKWebView）全绿；Windows（WebView2）经用户裁决**待后续补测**，不阻塞 beta.2 发布，补测结果回填本表。beta.2 收尾：legacy undo 路径已删除（回滚锚点 tag `pre-undo-cleanup`）→ tag `v3.0.0-beta.2` + onedir 内部包。
