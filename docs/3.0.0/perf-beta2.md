...
| 指标 | 目标 | 状态 |
|---|---|---|
| 1167 段列表滚动帧率 | ≥ 55 fps（WebView2 与 WKWebView 各实测） | 待冒烟（M7-2 窗口化 + v-memo 已就位） |
| 波形生成任务期主线程长任务 | >50ms 长任务为 0（DevTools Performance） | 待冒烟（M4 批量事件 + M6 rAF 合帧已就位） |
| 空闲 IPC 频率 | < 4 次/秒（自适应 tick 250ms 档） | 待冒烟 |
| 播放中 CPU 占用 | 对比 beta.1 下降（播放头 60Hz→≤10Hz 响应式写） | 待冒烟（M6 已就位） |
| 波形缓存二次打开 < 200ms | M11-3 落地后测 | 未开始（Phase 4） |
| 1167 段单字编辑重渲染行数 ≤ 可视区 | Vue DevTools 高亮验证（M7-1 v-memo 命中） | 待冒烟 |
| undo/redo ×5 手感 + macOS Cmd 链路 | 冒烟清单第 6 项 | 待冒烟 |
| M6-2 hover 预览手感 / M6 播放头无抖动 | 对比 beta.1 截录屏 | 待冒烟 |
