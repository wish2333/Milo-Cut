...
**验收标准 "WorkspacePage.vue < 40KB" 未达标：实际 61.3KB。** 五组归口（本步全部既定范围）已交付且全量门禁绿；差距 ~21KB 位于计划明示保留页内的区域（undo/键盘、popover/搜索/确认弹窗 UI、加载与 watch 接线）与模板本体。

**用户裁决（2026-08-31）：降级目标——接受 ~60KB 为 M8-2 里程碑，M8-2 关闭，按序进入 P3-3。** 备选路径（追加 M8-2d 子组件注入 / 模板再拆）不实施，如后续需要可在 v3.1 重启。
