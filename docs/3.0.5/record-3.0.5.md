...
- **P4-4 完成（R5.12 + D4 测试轮）+ beta.3 节点（P4 收官）**：prompt A 增 aligned_main_text 语义 1 行（:66-67，键集零影响）；D4 移交四项 9 例（detect_silence 本体 ×4 / 端到端串测 / padding=0 交叠 ×2 / 重叠段数据安全前后端双半）；**D4-2 发现并修复 add_silence_results 未排序真缺口**（1 行，排序不变量回归）；零反转；**tag `v3.0.5-beta.3`**；beta.3 期望超额达标（pytest **875** ≥863 / vitest **884·884 全绿** ≥864·863）；真机冒烟后置登记 record §5。
- **P5-1 完成（门禁终检与登记核对）**：全套门禁终跑 exit 0（pytest 875 / vitest 884·884 全绿，与 beta.3 一致只增不减）；登记表 ↔ diff 全对齐（六后端文件逐一有 R 编号；受控改点 (a)-(f) 六处全落位锚区间相符；main.py 四编号逐 hunk 在表）；断言反转终态 **10 行**（后端 4 / 前端 6，PLAN 预估为 P4 前估算值）全白名单内逐条落档；禁改面全空 + 两项登记不修项（MF2-1 / SG2-2）在档未误判。零代码改动，核对留痕 record-3.0.5-P5-1.md。
- **P5-2 完成（README 回填与版本池回写）**：README_zh 新段五条（增量补译/质量模式含 5× 时延/冻结矩阵中文首落/覆层三态/取消中性）+ README 英文镜像段；总记录新建 §7 版本池回写（出池 10 项表 / 新增 3 项 / 维持与观察项；**D3 经 R5.16 销账**）；零代码改动。
- **P5-3 待用户执行（★ 用户节点）+ 远程推送留痕**：`dev-3.0.5`（`68e619c`）与四个标签 `v3.0.5-base`/`beta.1`/`beta.2`/`beta.3` 已推送 origin（远端此前无 3.0.5 引用；push 后 0 ahead/0 behind 核验一致），测试机可拉取；**冒烟清单成文** [smoke-checklist-v3.0.5.md](./smoke-checklist-v3.0.5.md)——beta.1/2/3 三轮后置清单合并一次执行（B1-1~6 / B2-1~3 / B3-1~12 / RC-1~10），每项含操作 + 观测重点（**精确文案锚点已逐条与代码字面核对**：覆层 title/toast 族/失败指引/预估串/级联计数）+ 双平台签字表；RC-1 = 千段观测值回填债（本版必填）。plan P5-3 增执行依据指针。
- **P5-3 完成（★ 用户节点签字）+ 发布**：用户确认真机冒烟全量通过，准予发布——版本号 bump `3.0.4 -> 3.0.5`（pyproject + package.json 照 release 先例）；tag `v3.0.5` 落地；`gh pr create`（`dev-3.0.5` → `main`）+ `gh pr merge --merge` 留痕发布；RC-1 千段观测值债未回填数值，顺延登记 record-3.0.5-P5-3.md §3，不阻塞发布。
...
| P4-3 | record-3.0.5-P4-3.md | 已完成（R5.11+R5.15+R5.16 审阅/清理批；序 8 兑现；R5.16 豁免退役 vitest 全绿；门禁 exit 0） | `8503aa6` → merge `b78d23d` |
| P4-4 | record-3.0.5-P4-4.md | 已完成（R5.12 + D4 测试轮；D4-2 缺口修复；零反转；beta.3 tag 落地） | 代码 commit → merge（**tag `v3.0.5-beta.3`**） |
| P5-1 | （总记录 §3 核对 + 门禁终检留痕） | 已完成（门禁 exit 0；登记全对齐；反转 10 行全白名单；零代码改动） | record-3.0.5-P5-1.md（纯文档，`50061e0`） |
| P5-2 | （总记录 §7 版本池回写段） | 已完成（README ×2 回填；出池 10 / 新增 3 / D3 销账；零代码改动） | record-3.0.5-P5-2.md（纯文档，本 commit） |
| P5-2 | record-3.0.5-P5-2.md（§7 版本池回写段） | 已完成（README 回填与版本池回写） | 纯文档 `68e619c` |
| P5-3 | record-3.0.5-P5-3.md | 已完成（用户签字冒烟通过；bump + tag `v3.0.5` + gh PR 合入 main 发布） | 发布 commit → merge（tag `v3.0.5`） |
...
| P4-4 | core/llm_prompts.py | _SUBTITLE_CORRECTION_SYSTEM_A 增 aligned_main_text 语义说明 1 行（:66-67，edit_hint 块尾） | R5.12 | 受控增行 |
| P4-4 | core/project_service.py | add_silence_results 段列表排序（:1303-1309；D4-2 串测发现：静音与字幕交错时拼接非升序，排序不变量为前端渲染契约） | D4-2 | 修复 1 行 + 注释 |
| P4-4 | tests/test_d4_silence_gaps.py（新）+ SegmentBlocksLayer.test.ts | D4 移交四项 9 例（detect_silence 本体 ×4 / 端到端串测 / padding=0 交叠 ×2 / 重叠段数据安全前后端双半），不占 M-gate 额度 | D4 | 只增（测试） |

（后续 phase 按 SPEC M5.0-M5.8 触点表逐 hunk 登记；每条 diff 必须对应一个 R5.x 编号，无对应者补登记或回退。）

## 7. 版本池回写（P5-2，按 PRD §10 原文登记）

### 7.1 出池登记（10 项，登记处 → 本版交付）

| # | 项 | 原登记处 | 本版落位 |
|---|---|---|---|
| 1 | 翻译增量补译入口 | record-3.0.4 §7 新增 #3（版本池） | R5.3（P1-4）：缺口差集补译 + merge_translation_track 单 patch + 对账可读化 |
| 2 | 定稿译文滑动窗 | record-3.0.4 §7 新增 #5 | R5.8（P1-5）：quality_mode 串行 + 批 N 携批 N-1 定稿译文（窗口 1 批） |
| 3 | T1 方案 B 波形侧编辑模式一致性收口 | record-3.0.4 §7 新增 #1 | R5.7（P4-2）：TrackLane globalEditMode 守卫 + 冻结矩阵 + 主轨反向断言 |
| 4 | accept_high/clear track 作用域化 | record-3.0.4 §7 新增 #6 | R5.4（P2-1/P2-2）：三态作用域 + 聚合 patch + undo 三层 + 确认文案 |
| 5 | 纠错取消延迟（#1） | record-3.0.4 §8.1 | R5.5（P3-1）：1s 轮询 + 非阻塞 shutdown，MF2-1 判据冻结 |
| 6 | 批量作用域化（#2） | record-3.0.4 §8.1 | R5.4（同 #4） |
| 7 | 英文 tooltip（#4） | record-3.0.4 §8.1 | R5.9（P4-2）：Timeline title 中文化 + 覆层中文语义化 |
| 8 | prompt 语义说明（#5） | record-3.0.4 §8.1 | R5.12（P4-4）：aligned_main_text 一句受控增行 |
| 9 | D7 第三项 handleAcceptHighConfidence patch 化 | 散落登记（PRD §0.1 S2 行） | R5.4（P2-1/P2-2，同函数族顺带） |
| 10 | D3 perf 环境例根修（清债池） | record-3.0.3 顺延 → record-3.0.4 §8.1 | R5.16（P4-3）：**确定性重写完成，销账**——墙钟阈值断言移除、豁免口径退役、门禁全绿判（vitest 884/884） |

### 7.2 新增登记（3 项，3.1.x / 后续版本候选）

| # | 项 | 说明（PRD §10.2 原文） |
|---|---|---|
| 1 | S6 手动范围备注 | 3.1.x schema 演进首发候选（与工作区预设/行设置随工程同批重启）；降级诉求「建议面板条目 hover 展示创建时间」可独立评估 |
| 2 | 纠错侧取消 token 上报与中性提示对齐 | R5.1 边界；本版裁决 5 **明确不做**（B-6 口径：取消延迟修复与上报机制分属两族，收益/风险比劣于翻译侧）——若后续做，需给纠错侧 :1125/:1146/:1174 三处附 data 并统一中性 toast 族 |
| 3 | 补译进度逐批流式预览 | `llm:translation_progress` 登记项再评估：R5.3 交付后补译进度可顺带，本版不做 |

### 7.3 维持原样与观察项（照抄 PRD §10.2）

- **维持原样**：桥断连警示 / 点击字幕三模式 / 撤销恢复选区+视图 / 二分切片 / 手工 DOM 行保留；「需 schema 演进」两项与 S6 冻结至 3.1.x 专门重启；D1（InjectionKey 类型化）/ D2（悬空 pending 物理清理）/ D5（MiloCutApi 级写锁）/ D6（2.x 重叠段迁移观察项）/ D7 其余三项 / D8 余项。
- **观察项延续**：副轨删除确认策略（「无确认框 + undo 兜底」维持；R5.15 已补 toast 撤销提示，误删证据出现再议）；千段观测值回填（P5-3 真机轮必填）。
- **MF2-1 判据不对称**（本版登记不修，P3-1 record §5）：纠错两相记账判据统一需与 7.2#2 同族施工，3.1.x 评估。
