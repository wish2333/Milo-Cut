...
- [x] R5.12：`_SUBTITLE_CORRECTION_SYSTEM_A`（:51）末尾增 aligned_main_text 语义说明一句（受控增行；注册表键集不变零改动——区间实测 :157-194；键集类测试如受影响按 3.0.4 §4.1 追认制登记）——落位 :66-67 edit_hint 块尾，键集测试零影响
- [x] D4 四项测试轮（PRD §7.4 移交落点；**不占 ≥30/≥24 额度、增量如实登记**）：detect_silence 本体 / 端到端串测 / padding=0 交叠 / basic 空白点击建重叠段（兜数据安全，优先补）——9 例（后端 8 + 前端 1）；**D4-2 串测发现并修复 add_silence_results 未排序真缺口**（1 行，add_segment 先例）
- [x] ★ beta.3 节点：P4 末全套门禁（期望 ≥863 / ≥864·863 → 实际 **875 / 884·884 全绿**）+ 打 tag `v3.0.5-beta.3` + record；★ 真机冒烟按 beta.3 轮清单（后置先例同前，record §5）
...
- [x] README_zh / README 3.0.5 功能段：增量补译与缺口语义 / 质量模式（含约 5× 时延标注）/ trim 冻结矩阵（含主/副轨不对称理由）/ 覆层三态语义 / 取消中性提示——中文新段五条 + 英文镜像段（冻结矩阵英文条 P4-2 已落 v3.0.4 段尾不搬移，record §3 注）
- [x] 版本池回写（PRD §10）：出池登记（版本池 4 + record §8.1 4 + 散落 1 + 清债池 1——总记录 §7.1 十项表）；新增登记 ① S6（3.1.x）/ ② 纠错侧上报不做（B-6 口径留档）/ ③ 补译进度流式；D3 处置 = R5.16 完成则销账——**销账落地**（豁免退役 + 全绿判生效）
...
**执行依据（2026-09 成文）**：[smoke-checklist-v3.0.5.md](./smoke-checklist-v3.0.5.md)——beta.1/beta.2/beta.3 三轮后置清单合并为一次执行（3.0.4 冒烟后置先例），共 B1-1~6 / B2-1~3 / B3-1~12 / RC-1~10，每项含操作步骤 + **观测重点（精确文案锚点，已与代码字面逐条核对）** + §7 双平台签字表；RC-1 = 千段观测值回填债（本版必填）。
