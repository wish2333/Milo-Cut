...
<template>
  <div class="flex h-screen flex-col bg-parchment">
    <!-- Top navigation -->
    <div class="flex items-center gap-3 border-b border-hairline bg-canvas px-4 py-3">
      <button
      ...
      <div class="flex-1" />
      <span v-if="statusMessage" class="text-sm text-primary">{{ statusMessage }}</span>
      <span v-if="errorMessage" class="text-sm text-status-warning">{{ errorMessage }}</span>
      <span v-if="confirmedEdits.length > 0" class="text-sm text-ink-muted">
    ...
    <div v-if="isExporting && exportProgress" class="border-b border-hairline bg-canvas px-4 py-2">
      <div class="flex items-center gap-3">
        <div class="h-2 flex-1 overflow-hidden rounded-full bg-hairline">
          <div
            class="h-full rounded-full bg-primary transition-all duration-300"
            :style="{ width: `${exportProgress.percent ?? 0}%` }"
          />
        </div>
        <span class="shrink-0 text-xs text-ink-muted">{{ exportProgress.message ?? '' }}</span>
      </div>
    </div>

    <div class="flex flex-1 overflow-hidden">
      <!-- Left: Settings panel -->
      <div class="w-80 overflow-y-auto border-r border-hairline bg-canvas p-4">
        <h3 class="mb-3 font-semibold">导出设置</h3>
      ...
      <div class="w-80 overflow-y-auto border-l border-hairline bg-canvas p-4">
        <h3 class="mb-3 font-semibold">导出选项</h3>

        <div class="space-y-3">
          <button
            class="mc-button mc-button-primary w-full px-4 text-sm font-semibold"
          ...
          <div class="mt-3 border-t border-hairline pt-3">
            <h4 class="mb-2 text-xs font-semibold text-ink-muted">
              精华导出
              <span v-if="highlightCount > 0" class="text-ink-muted">({{ highlightCount }} 个片段)</span>
            </h4>
            <div class="space-y-2">
              <button
                class="mc-button mc-button-secondary w-full px-4 text-sm font-semibold"
              ...
              <p v-if="highlightCount === 0" class="text-xs text-ink-muted">
          ...
          <div class="mt-3 border-t border-hairline pt-3">
            <h4 class="mb-2 text-xs font-semibold text-ink-muted">过渡设置</h4>
            <div :class="{ 'opacity-50 pointer-events-none': otioExportMode === 'full_timeline' }">
              <label class="block">
                ...
                <input
                  ...
                  class="w-full mt-1"
                />
                <p v-if="otioFadeDuration > 0" class="mt-0.5 text-xs text-ink-muted">
          ...
          <!-- Timeline formats -->
          <div class="mt-3 border-t border-hairline pt-3">
            <h4 class="mb-2 text-xs font-semibold text-ink-muted">时间线格式</h4>
            <button
              class="mc-button mc-button-secondary w-full px-4 text-sm font-semibold"
              :disabled="isExporting"
            ...
            <button
              class="mc-button mc-button-secondary mt-2 w-full px-4 text-sm"
              :disabled="isExporting"
        ...
        <div class="mt-6 border-t border-hairline pt-4">
          <h4 class="mb-2 text-xs font-semibold text-ink-muted">项目信息</h4>
